...
export class AndroidOperator extends Operator {
  ...
  public async screenshot(): Promise<ScreenshotOutput> {
    ...
    try {
      ...
      logger.info('[AndroidOperator] screencap', screencap.stdout);

      // 将截图拉取到本地临时目录
      const pullScreenshot = await commandWithTimeout(
        `adb -s ${this.deviceId} pull /sdcard/uitars_screenshot.png`,
      ).catch(() => ({
        stdout: '',
      }));
      logger.info('[AndroidOperator] pullScreenshot', pullScreenshot.stdout);

      // 读取图片并获取尺寸
      const image = await Jimp.read('uitars_screenshot.png');
      ...
      logger.info(
        `[AndroidOperator] screenshot: ${width}x${height}, scaleFactor: 1`,
      );
      ...
      logger.error('[AndroidOperator] Screenshot error:', error);
      throw error;
    }
  }
  ...
  async execute(params: ExecuteParams): Promise<ExecuteOutput> {
    ...
    try {
      switch (action_type) {
        ...
        default:
          logger.warn(`[AndroidOperator] Unsupported action: ${action_type}`);
          break;
      }
    } catch (error) {
      logger.error('[AndroidOperator] Error:', error);
      throw error;
    }
  }
}
