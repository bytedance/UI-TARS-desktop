...
export const BrowserControlDisplay: React.FC<BrowserControlDisplayProps> = ({ mode, tools }) => {
  ...
  const formatModeName = (mode: string): string => {
    switch (mode) {
      case 'default': return 'Hybrid Browser Control';
      case 'browser-use-only': return 'DOM-based Browser Control';
      case 'gui-agent-only': return 'Vision-based Browser Control';
      default: return `Browser Control: ${mode}`;
    }
  };
  ...
  const getModeIcon = () => {
    switch (mode) {
      case 'default': return <FiSettings className="text-purple-500" />;
      case 'browser-use-only': return <FiCode className="text-blue-500" />;
      case 'gui-agent-only': return <FiEye className="text-green-500" />;
  ...
  const groupedTools = React.useMemo(() => {
    ...
    tools.forEach(tool => {
      if (tool.includes('navigate') || tool.includes('back') || tool.includes('forward') || tool.includes('refresh') || tool.includes('tab')) {
        groups.navigation.push(tool);
      } else if (tool.includes('click') || tool.includes('type') || tool.includes('press') || tool.includes('hover') || tool.includes('drag') || tool.includes('scroll')) {
        groups.interaction.push(tool);
      } else if (tool.includes('get_markdown') || tool.includes('get_html') || tool.includes('get_text')) {
        groups.content.push(tool);
      } else if (tool.includes('screenshot') || tool === 'browser_vision_control') {
        groups.visual.push(tool);
      } else if (tool.includes('get_url') || tool.includes('get_title') || tool.includes('get_elements')) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'navigation': return <FiMonitor size={12} className="text-blue-500" />;
      case 'interaction': return <FiSettings size={12} className="text-purple-500" />;
      case 'content': return <FiCode size={12} className="text-amber-500" />;
      case 'visual': return <FiEye size={12} className="text-green-500" />;
      case 'status': return <FiInfo size={12} className="text-cyan-500" />;
      default: return <FiSettings size={12} className="text-gray-500" />;
    }
  };
          ...
          {Object.entries(groupedTools).map(([category, categoryTools]) => (
            <div key={category}>
              <div className="flex items-center mb-1.5 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <span className="ml-1 text-gray-400 dark:text-gray-500">({categoryTools.length})</span>
              </div>
