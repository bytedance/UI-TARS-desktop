...
and Agent TARS additionally exposes a `web_scrape` tool when Firecrawl is the
configured provider.
...
- `web_scrape` — fetch any single URL as clean markdown without navigating the
...
single provider covers both motions, and `web_scrape` is a drop-in for the
...
| **scrape** | ✅ shipped | `web_scrape` tool — read any URL (incl. JS-rendered pages and PDFs) to markdown without driving the browser. |
...
costs per result (1 credit/page basic). `web_scrape` is 1 credit/page. Keep
`scrapeOptions` off `web_search` unless you need content from *every* result;
otherwise search first, then `web_scrape` only the URLs you want.
