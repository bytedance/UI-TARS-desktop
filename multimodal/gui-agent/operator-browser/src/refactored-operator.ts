  ...
  protected async execute(params: ExecuteParams): Promise<ExecuteOutput> {
    ...
    for (const action of actions) {
      if (this.showActionInfo) {
        ...
        await this.uiHelper?.showActionInfoByAction(action, reasoningContent ?? '');
      }
  ...
  private async singleActionExecutor(action: BaseAction): Promise<ExecuteOutput> {
    ...
    try {
      ...
      switch (action_type) {
        ...
        case 'navigate':
          if (!action_inputs.url) {
            // TODO: if throw error, should be handled by agent
            throw new Error('No target url specified for navigation');
          }
          await this.handleNavigate({ url: action_inputs.url });
          break;
        case 'navigate_back':
        ...
        case 'left_click':
        case 'left_single':
          if (action_inputs.point) {
            const { realX: startX, realY: startY } = await this.calculateRealCoords(
              action_inputs.point,
            );
            await this.handleClick(startX, startY);
          }
          // else throw new Error(`Missing startX(${startX}) or startY${startX}.`);
          else throw new Error(`Missing point for click.`);
          break;
        ...
        case 'left_double':
          ...
          } else throw new Error(`Missing point for double click.`);
          break;
        ...
        case 'right_click':
          ...
          } else throw new Error(`Missing point for right click.`);
          break;
