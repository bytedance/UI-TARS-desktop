...
export const setActiveSessionAction = atom(null, async (get, set, sessionId: string) => {
  try {
    ...
    // One-time status recovery for page refresh scenarios
...
// Removed checkSessionStatusAction - now fully relies on SSE events for real-time status updates
// Session status recovery is handled in setActiveSessionAction for page refresh scenarios
