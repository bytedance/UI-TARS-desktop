...
|------|----------|-------------|
| [`initialize()`](#initialize) | Lifecycle | Called during agent initialization |
| [`onDispose()`](#ondispose) | Lifecycle | Called during agent disposal |
| [`onLLMRequest()`](#onllmrequest) | LLM Interaction | Before sending request to LLM |
| [`onLLMResponse()`](#onllmresponse) | LLM Interaction | After receiving response from LLM |
| [`onLLMStreamingResponse()`](#onllmstreamingresponse) | LLM Interaction | For streaming responses from LLM |
| [`onBeforeToolCall()`](#onbeforetoolcall) | Tool Execution | Before executing a tool |
| [`onAfterToolCall()`](#onaftertoolcall) | Tool Execution | After executing a tool |
| [`onToolCallError()`](#ontooolcallerror) | Tool Execution | When tool execution fails |
| [`onProcessToolCalls()`](#onprocesstoolcalls) | Tool Execution | Intercept tool call processing |
| [`onEachAgentLoopStart()`](#oneachagentloopstart) | Loop Control | Start of each agent loop iteration |
| [`onEachAgentLoopEnd()`](#oneachagentloopend) | Loop Control | End of each agent loop iteration |
| [`onAgentLoopEnd()`](#onagentloopend) | Loop Control | When entire agent loop completes |
| [`onBeforeLoopTermination()`](#onbeforelooptermination) | Loop Control | Before agent loop terminates |
| [`onPrepareRequest()`](#onpreparerequest) | Request Preparation | Before preparing LLM request |
