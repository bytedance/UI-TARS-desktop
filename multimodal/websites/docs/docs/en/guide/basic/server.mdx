...
Agent TARS Server 是 Agent TARS 生态中基于 Agent TARS Core 构建的服务端，其包含会话存储管理、Web UI 等能力：
...
<Figure src="/agent-tars-serve.png" title="Figure 1: Agent TARS Headless Server" />
