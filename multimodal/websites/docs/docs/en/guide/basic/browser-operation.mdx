...
可以看到，Agent 能够看到，并直接基于视角完成点击、输入，很快就完成任务了：
...
本质上，是由模型完成 Grounding，返回具体要操作的坐标和内容，然后交给 Browser Operator 完成执行，上述任务实际上经历了三次输出，最终解析后的结果如下：
...
从本质来说，Hybrid 模式是将两种场景的 Action Space 合并，通过 Prompt Engineering 协调关系，并并交给模型最终来推理和协调合适应当选择何种工具。由于我们已经为 [Visual Grounding](#visual-grounding) 增加了 “信息提取” 相关的工具，因此，大多数情况下，目前 Hybrid 的表现和 [Visual Grounding](#visual-grounding) 的实际表现已经差距不大。

在一些场景下，Hybrid 模式下，可以在尝试先试用更轻量的 [DOM](#dom) 操作方案，如果失败了，[Visual Grounding](#visual-grounding) 能够进行兜底：
...
可以看到，基于 DOM 的方案本质上相当于模态信息的缺失，因此对模型的推理能力要求较高，因此这套方案适用于偏文字类的场景，如。
