...
“Building Long-running Agents” 一直是 Agent TARS 的长期目标之一，以下面的例子为例：
...
没错，Agent Event Stream 在内部也用于构建 Context，没错，就是前文提到的 [Context Engineering](#context-engineering)，是不是都串起来了？关于 Agent Event Stream 的细节，我们回头会单独写一篇 Blog 来介绍。
...
在 Agent TARS 的早期预览版本中，我们只支持 Claude 3.7，但仍然受到社区对各种模型兼容的大量请求，我们也发起了关于 [Model Compatibility #377](https://github.com/bytedance/UI-TARS-desktop/discussions/377) 的 Discussion。

在早期版本中，受限于当时的架构，我们并没有完成一个很好的模型兼容，但从 Beta 开始，我们完全重构了 Model Provider 层的实现，终于可以说明我们拥有更好的模型兼容性，现在的 Agent TARS，可以在 Volcengine、Anthorpic’、OpenAI 上运行。
