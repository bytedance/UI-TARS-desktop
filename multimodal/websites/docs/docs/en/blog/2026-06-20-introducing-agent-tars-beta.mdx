...
::: tip Responses API除此之外，我们正在计划在 [Chat Completion](https://platform.openai.com/docs/api-reference/chat) 的基础上，让 Agent 的 LLM API Protocol 继续支持 [Responses API](https://platform.openai.com/docs/api-reference/responses)，利用 LLM Serving 的 Image Cache 进一步提升多模态推理任务场景下的性能。:::
...
[AG-UI](https://docs.ag-ui.com/introduction) 是一个前沿的协议，旨在通过开放协议，标准化前端应用与 AI Agent 的连接方式。这和上述的 Agent Event Stream 看起来很像？没错，是的，在 AG-UI 发布时，我们就关注到这一 protocol，并对齐进行了充分地调研与学习，我们发现 AG-UI Procotol 有不少值得学习的地方，如 [ State Management Events](https://docs.ag-ui.com/sdk/js/core/events#state-management-events)，基于 [JSON Patch format (RFC 6902)](https://datatracker.ietf.org/doc/html/rfc6902) 的 Incremental state changes ，做到了更自然的 `START-CONTENT-END` 增量状态更新。
...
好了，以上我们向你阐述了我们在构建 Agent TARS Beta 中的一些关键认知，他将对你理解这次发布非常关键。接下来，我们将向你正式介绍 Agent TARS Beta 带来的能力上的变化。
...
从 Beta 开始，Agent TARS 的应用形态从一个 Electron App 演进为一个 CLI，并带来了一个针对 [新架构](#archtecutre-improvement) 的新版本 Web UI：
...
<Grid columns={2} gap="md">
  <GridItem>
    ...
    **为什么选择 CLI？**
    ...
    这主要源于 CLI 在不损失核心能力的前提下，还具有如下优势：
    
    1. **更容易更新迭代**：目前 `@agent-tars/cli` 已经迭代 35 个版本，远超早期 Electron 的 9 个版本，这让我们能够更快地修复用户遇到的问题；
    2. **安装体积具有显著优势**：这主要是 Agent TARS CLI 无需打包一个 [Chromium](https://www.chromium.org/)，具体的体积优势如右图所示。

    有关 `@agent-tars/cli` 如何使用 ，请移步 [Quick Start](../guide/get-started/quick-start.mdx)。

  </GridItem>
  <GridItem>
    <Figure src="/compare-install-size.png" title="Figure 1: Agent Loop Memory" zoom={1}/>
  </GridItem>
</Grid>

::: tip 未来还会提供 Electron App 吗？
...
在 Agent TARS 最新的版本中，我们在早期基于 DOM 的 Browser 控制的方案基础上，引入了基于 VLM，以及 DOM + VLM 两种模式混合的三种浏览器操控方案：

| 术语 | 介绍 |
| --- | --- |
| `dom` | 操作能力基于 DOM 分析的 [Browser Use](https://www.npmjs.com/@agent-infra/mcp-server-browser)，在 Agent TARS 早期版本为此方案。 |
| `visual_grounding` | 操作能力基于 GUI Agent（[UI-TARS](https://seed-tars.com/) / [Doubao 1.5 VL](https://www.volcengine.com/docs/82379/1554521)），无 DOM 相关的工具。 |
| `hybrid` | 操作能力同时包含 `visual_grounding` 与 `dom` |

> \* _三种模式均包含 *`navigate`*、*`tab`* 相关的基础导航工具。_
...
在 Agent TARS 的早期预览版本中，我们只支持 Claude 3.7，但仍然受到社区对各种模型兼容的大量 Request，为此，我们也发起了 关于 [Model Compatibility #377](https://github.com/bytedance/UI-TARS-desktop/discussions/377) 的 Discussion。

在早期版本中，受限于当时的架构，我们并没有完成一个很好的模型兼容，但从 Beta 开始，我们终于可以说明我们拥有更好的模型兼容性，现在的 Agent TARS，可以在 Volcengine、Anthorpic’、OpenAI、
