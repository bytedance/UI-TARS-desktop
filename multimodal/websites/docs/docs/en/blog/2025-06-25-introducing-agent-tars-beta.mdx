...
> _\* You can find all these past moments on our [official Twitter](https://x.com/AgentTars)._
...
<Figure src="/agent-loop-memory.png" title="Figure 1: Agent Loop Memory" zoom={1.4} maxWidth="70%" />
...
<Figure
  ...
  title="Figure 2: Typical model context window sizes (as of 2025.5)"
  zoom={1.3}
  maxWidth="50%"
/>
...
<Figure
  ...
  title="Figure 3: How MCP provides context for Agent"
  zoom={1.4}
  maxWidth="75%"
/>
...
<Figure src="/hierarchical-context.png" title="Figure 4: Hierarchical context" zoom={1.3} maxWidth="80%" />
...
<Figure
  ...
  title="Figure 4: Complex and constantly changing environment"
  zoom={1.3}
  maxWidth="80%"
/>
...
In addition, we are also building some basic evaluation sets for MCP Tools, which are currently partially evaluating the cross-model compatibility performance of MCP Agent within Agent TARS. Once these capabilities are fully improved, we will introduce a complete Benchmark solution in the official version.
...
For how to use and more details, please visit [Browser Operation](../guide/basic/browser-operation.mdx).
...
Based on the [understanding](#understanding) above, we completely rewrote Agent TARS Beta, bringing a brand-new multi-layered architecture built on an Event Stream-driven Agent Kernel.
...
Agent TARS is still developing at a rapid pace, and we have cutting-edge capabilities still in internal testing. In the future, we expect Agent TARS to truly become an Agent development tool that everyone can use anytime, anywhere. Please look forward to our updates, and thank you to all community members who have supported us ❤️
