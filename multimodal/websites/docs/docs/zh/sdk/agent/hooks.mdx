...
| Hook | 分类 | 描述 |
|------|------|------|
| [`initialize()`](#initialize) | 生命周期 | agent 初始化期间调用 |
| [`onDispose()`](#ondispose) | 生命周期 | agent 销毁期间调用 |
| [`onLLMRequest()`](#onllmrequest) | LLM 交互 | 向 LLM 发送请求之前 |
| [`onLLMResponse()`](#onllmresponse) | LLM 交互 | 从 LLM 接收响应之后 |
| [`onLLMStreamingResponse()`](#onllmstreamingresponse) | LLM 交互 | LLM 流式响应 |
| [`onBeforeToolCall()`](#onbeforetoolcall) | Tool 执行 | 执行 Tool 之前 |
| [`onAfterToolCall()`](#onaftertoolcall) | Tool 执行 | 执行 Tool 之后 |
| [`onToolCallError()`](#ontooolcallerror) | Tool 执行 | Tool 执行失败时 |
| [`onProcessToolCalls()`](#onprocesstoolcalls) | Tool 执行 | 拦截 Tool 调用处理 |
| [`onEachAgentLoopStart()`](#oneachagentloopstart) | 循环控制 | 每个 agent 循环迭代开始 |
| [`onEachAgentLoopEnd()`](#oneachagentloopend) | 循环控制 | 每个 agent 循环迭代结束 |
| [`onAgentLoopEnd()`](#onagentloopend) | 循环控制 | 整个 agent 循环完成时 |
| [`onBeforeLoopTermination()`](#onbeforelooptermination) | 循环控制 | agent 循环终止之前 |
| [`onPrepareRequest()`](#onpreparerequest) | 请求准备 | 准备 LLM 请求之前 |
