...
在三月份，我们开源了 Agent TARS 的早期[技术预览版](https://agent-tars.com/2025/03/18/announcing-agent-tars-app)。Agent TARS 的口号是<u>"一个开源的多模态 AI 代理，提供与各种现实世界工具的无缝集成"</u>。在发布后，它获得了社区的一些支持并积累了一定的影响力。
...
2. [可观测和可评估](#observable-and-evaluable)
3. [易于构建应用](#easy-to-build-applications)

<br />

### Context Engineering

"构建长时间运行的代理"一直是 Agent TARS 的长期目标之一，以下面的例子为例：
...
Agent 经过二十轮左右完成了任务，尤其是在多模态任务下，如果没有细致的 Context Engineering，上下文非常容易溢出。在 Agent TARS 中，Context Engineering 体现在以下几个方面：
...
<Figure src="/agent-loop-memory.png" title="图 1: Agent Loop Memory" zoom={1.4} maxWidth="70%" />
...
<Figure
  ...
  title="图 2: 典型模型的上下文窗口大小 (截至 2025.5)"
  zoom={1.3}
  maxWidth="50%"
/>
...
<Figure src="/hierarchical-context.png" title="图 4: 分层上下文" zoom={1.3} maxWidth="80%" />
...
除此之外，我们同时在建设一些针对 MCP Tools 的基础评测集，目前已经部分在评测 Agent TARS 内部的 MCP Agent 的跨模型兼容表现，在这些能力全部完善后，我们会在正式的版本中推出完整的 Benchmark 方案。
...
这样的好处不止在于能够让 Agent 与 UI 遵守某些协议协议，如果你觉得这些 UI 不好看，你也能够去替换这些 UI 的实现 —— 没错，这也是 Agent TARS 未来的愿景之一，支持社区来来定义不同的 Agent TARS UI 实现。
...
没错，Agent Event Stream 在 Agent TARS 内部也用于构建 Context，没错，就是前文提到的 [上下文工程](#context-engineering)，是不是都串起来了？关于 Agent Event Stream 的细节，我们回头会单独写一篇 Blog 来向大家介绍。
...
<Figure src="/compare-install-size.png" title="图 1: Agent Loop Memory" zoom={1} />

有关 `@agent-tars/cli` 的使用指南，请移步 [Quick Start](../guide/get-started/quick-start.mdx)。
...
有关如何使用和更多细节，请移步 [Browser Operation](../guide/basic/browser-operation.mdx)。
...
基于上述[认知](#认知)，我们将 Agent TARS Beta 进行了完全的重写，带来了全新的多层分层架构，其核心构建在一个基于 Event Stream 驱动的 Agent Kernel 上。

总的来说，Agent TARS 的主要组成部分如下：
...
<p align="center" className="text-sm">
  <span>图 1: Agent TARS Beta 的架构</span>
</p>
...
除了在上述介绍特性中演示的例子外，这里有一些内部开发者演示的例子，虽然这些能力并非 Agent TARS 正式支持的能力，但得益于模型能力的持续提升，以及 [Context Engineering](#context-engineering) 的持续演进，

### 复刻应用

Agent TARS 暂不支持 Coding，亦不支持 Artifact Preview，但 Agent TARS 支持多模态输入，你可以通过 File 工具写代码，Browser 实现预览：
...
类似的，甚至有开发者通过 Agent TARS 自主的多轮迭代，得到了几乎是专业效果的 UI 输出：
...
这是另一个 Agent TARS 一直以来比较关注的例子，如何生成多模态的内容：
...
Agent TARS 仍在飞速的发展之中，我们还有前沿的能力仍在内测之中，未来，我们期望 Agent TARS 能够真正地成为大家随时随地可用的 Agent 开发者工具。请期待我们的更新，感谢所有支持过我们的社区同学 ❤️
