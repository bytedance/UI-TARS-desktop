...
Agent TARS 框架设计了一个模型提供者机制，允许你自由使用不同的模型。

#### 概述

目前 Agent TARS 的模型兼容性状态如下：

| 模型提供者 | 模型             | 文本 | 视觉 | 工具调用 & MCP | 视觉定位 |
| ------------- | ----------------- | ---- | ------ | --------------- | ---------------- |
...
**[Seed1.5-VL](https://raw.githubusercontent.com/ByteDance-Seed/Seed1.5-VL/refs/heads/main/README.md)** 是一个强大且高效的视觉语言基础模型，设计用于高级通用多模态理解和推理，Seed1.5-VL 已部署在[火山引擎](https://www.volcengine.com/product/doubao)上，模型 ID 为 `doubao-1-5-thinking-vision-pro-250428`。

一旦你获得了 `API_KEY`，你可以使用单个命令启动 Agent TARS：
...
有关模型支持的更多详情，请查看[模型提供者](../basic/model-provider.md)。
...
虽然你可以使用 CLI 快速启动，但我们仍然建议创建一个全局工作区。这允许你通过配置文件维护配置，以及存储你的[文件系统](../basic/file.md)。让我们开始：
...
接下来，你可以在全局工作区中使用 TypeScript 维护配置，而不是处理 CLI 参数，并且受益于完整的类型检查：
...
有关完整配置详情，请前往[配置](../basic/config.md)，有关模型配置，请前往[模型提供者](../basic/model-provider.md)。
