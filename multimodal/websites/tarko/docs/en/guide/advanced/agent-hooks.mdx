...
Agent Hooks are callback methods that execute at specific points during agent operation. All hooks are defined in the `BaseAgent` class and can be overridden in custom agent implementations:
...
```typescript
import { Agent, BaseAgent } from '@tarko/agent';
...
```typescript
class ToolMonitoringAgent extends BaseAgent {
...
```typescript
class LoopControlAgent extends BaseAgent {
...
```typescript
import { BaseAgent, AgentOptions } from '@tarko/agent';
