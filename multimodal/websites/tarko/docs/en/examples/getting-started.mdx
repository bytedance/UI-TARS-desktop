...
const calculatorTool = new Tool({
  ...
  parameters: z.object({
    expression: z.string().describe('Mathematical expression (e.g., "2 + 3 * 4", "sqrt(16)", "sin(pi/2)")')
  }),
...
const unitConverterTool = new Tool({
  ...
  parameters: z.object({
    ...
    fromUnit: z.string().describe('Source unit (e.g., "celsius", "fahrenheit", "meters", "feet", "kg", "lbs")'),
...
describe('Calculator Agent', () => {
  it('should perform basic calculations', async () => {
    const response = await calculatorAgent.run({
      input: 'What is 15 * 7?'
    });
    expect(response.content).toContain('105');
  });
  ...
  it('should convert units', async () => {
    ...
    expect(response.content).toContain('37.7778');
  });
});
```
